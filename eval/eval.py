...
def main(parser):
    ...
    for layer in layers:
        # Load data
        attack = torch.load(f"../model/output_tensors_ai/{model_name_base}_all_layer_{layer}_perturbed_text.pt",
                            map_location=device, weights_only=True)
        original = torch.load(f"../model/output_tensors_ai/{model_name_base}_all_layer_{layer}_original_text.pt",
                              map_location=device, weights_only=True)
